...
def draw_wuxing_wheel_roulette(scores: dict, rotation_deg: float = 0.0):
    """
    五行转盘（轮盘风格）
    - rotation_deg: 旋转角度（度），用来做“转动动画”
    """
    labels = list(scores.keys())  # ["木","火","土","金","水"]
    ...
    ax.set_theta_offset(np.pi/2)   # 0 度在正上方
    ax.set_yticklabels([])
    ax.set_xticklabels([])
    ax.grid(False)
    ...
    ax.set_title("五行轮盘（娱乐版）", pad=20, fontsize=16)
...
def page_bazi(uid: str):
    st.subheader("🎴 八字 / 五行轮盘（娱乐版）")
    d = st.date_input("出生日期")
    t = st.time_input("出生时间（可大概）")
...
# 初始化轮盘状态
    ...
    colA, colB = st.columns([1, 1])
    with colA:
        if st.button("🎡 开始轮盘推演（更有仪式感）", use_container_width=True):
            # 做一个“伪随机但稳定”的停点：跟生日时间绑定，这样每次打开结果一致（更像“命盘固定”）
            seed = f"{d}|{t}|{gender}"
            h = int(hashlib.md5(seed.encode("utf-8")).hexdigest()[:8], 16)
            target = (h % 360)  # 0~359 度

            # 动画：从当前角度开始，快速转几圈再慢慢停
    ...
    with colB:
        st.caption("提示：这是“娱乐版轮盘仪式感”，不是真实排盘。结果会跟生日时间绑定，体验更像固定命盘。")

    # 转完后展示结果摘要
    main1, main2 = _dominant_two(scores)
    if st.session_state.bazi_spun:
        st.success(f"轮盘已定格：主五行偏向 **{main1[0]}**，其次 **{main2[0]}**（仅娱乐）")
    else:
        st.info("你可以先点一次【开始轮盘推演】再生成解读。")


    if st.button("✨ 生成八字/五行解读（消耗 1 次）", use_container_width=True):
      if not st.session_state.bazi_spun:
          st.warning("建议先点【开始轮盘推演】让轮盘定格，再生成解读（更有仪式感）。")
          st.stop()
